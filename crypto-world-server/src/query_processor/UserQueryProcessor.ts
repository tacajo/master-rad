...
export class UserQueryProcessor {
  ...
  public static async registerUser(
    ...
    let query =
      'INSERT INTO public."user" (first_name, last_name, registration_date, company, banned, balance, role, birthday, title_in_the_company, email, password) VALUES ($1, $2, to_timestamp(($3) / 1000.0), $4, $5, $6, $7, $8, $9, $10, $11)  RETURNING *';
    let result = await PostgresDB.client.query(query, [
      firstName,
      lastName,
      Date.now(),
  ...
  public static async payToken(id: number, amount: number) {
    let query =
      'UPDATE public."user" SET balance = (balance - $1) WHERE user_id = $2;';
  ...
  public static async addTokensToUser(
    user_id: number,
    token_id: number,
    amount: number
  ) {
    let query =
      "UPDATE public.portfolio SET amount = amount + $1 WHERE user_id = $2 and token_id = $3;";
    let result = await PostgresDB.client.query(query, [
      amount,
      user_id,
      token_id,
  ...
  public static async removeTokensToUser(
    user_id: number,
    token_id: number,
    amount: number
  ) {
  ...
  public static async addSubscription(
    ...
    let query =
      "INSERT INTO public.subscription (subscription_id, user_id, order_id) VALUES ($1, $2, $3)  RETURNING *";
    let result = await PostgresDB.client.query(query, [
      subscription_id,
      user_id,
      order_id,
